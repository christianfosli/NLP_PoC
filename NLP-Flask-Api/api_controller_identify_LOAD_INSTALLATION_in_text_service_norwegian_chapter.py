...
def create_api_response_for_post_identify_LOAD_INSTALLATION_in_text_service_norwegian_chapter(title_dictionary,forward_filtered_result_with_only_the_things_we_are_looking_for):
    ...
    for line in forward_filtered_result_with_only_the_things_we_are_looking_for:
        ...
        for ent_index_number, ent in enumerate(ents):
            ...
            if ent_label == "LOAD_INSTALLATION":

                detection_with_url_metadata = dict(metadata_from_url)
                detection_with_url_metadata["load_installation_text"] = ent_text

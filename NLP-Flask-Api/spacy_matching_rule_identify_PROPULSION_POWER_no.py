...
def identify_PROPULSION_POWER_in_text(text):
    nlp = English()
    doc = nlp(text)
    ...
    matcher.add(
        ...
        [
            [
                {"LOWER": {"IN": ["framdriftskraft"]}}
            ]
        ])

    matcher.add(
        "PROPULSION_POWER_FACT",
        [
            [
                {'IS_DIGIT': True},
                {"LOWER": {"IN": ["kw"]}}
    ...
    for match_id, token_start, token_end in matcher(doc):
        ...
        if (match_id_as_string == "PROPULSION_POWER_FACT" and
            (token_end + 1) < tokens_in_doc_count):

            next_word_1_token_number = token_end
            next_word_1_token = doc[next_word_1_token_number]
            next_word_2_token_number = token_end + 1
            next_word_2_token = doc[next_word_2_token_number]

            if (next_word_1_token.text.lower() == "eller" and
                next_word_2_token.text.lower() == "mer"):
                # Expanding.
                final_token_end = next_word_1_token_number + 2
...
def identify_PROPULSION_POWER_in_norwegian_spacy_lines(spacy_lines):
    ...
    for spacy_line in spacy_lines:
        ...
        if spacy_line['text'] != "\n" and len(spacy_line['text']) > 0:
            ...
            nlp_result_list = identify_PROPULSION_POWER_in_text(spacy_line['text'])

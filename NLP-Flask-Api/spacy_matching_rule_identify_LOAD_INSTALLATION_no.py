...
def identify_LOAD_INSTALLATION_in_text(text):
    nlp = English()
    doc = nlp(text)
    ...
    for match_id, token_start, token_end in matcher(doc):
        ...
        if match_id_as_string == "LOAD_INSTALLATION" and token_end <= tokens_in_doc_count:

            next_word_1_token_number = token_end
            next_word_1_token = doc[next_word_1_token_number]

            if next_word_1_token.text.lower().startswith("losseinnretning"):
                # Expanding.
...
def identify_LOAD_INSTALLATION_in_norwegian_spacy_lines(spacy_lines):
    ...
    for spacy_line in spacy_lines:
        ...
        if spacy_line['text'] != "\n" and len(spacy_line['text']) > 0:
            ...
            nlp_result_list = identify_LOAD_INSTALLATION_in_text(spacy_line['text'])

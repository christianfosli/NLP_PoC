...
def identify_FLASHPOINT_in_text(text):
    nlp = English()
    doc = nlp(text)
...
def identify_FLASHPOINT_in_norwegian_spacy_lines(spacy_lines):
    ...
    for spacy_line in spacy_lines:
        ...
        if spacy_line['text'] != "\n" and len(spacy_line['text']) > 0:
            ...
            nlp_result_list = identify_FLASHPOINT_in_text(spacy_line['text'])
